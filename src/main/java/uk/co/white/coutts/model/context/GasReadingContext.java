...
import uk.co.white.coutts.model.data.GasReading;

public class GasReadingContext extends AbstractConnection implements ReadingContext
{
	
	private String query = "SELECT * FROM readings "
			+ "JOIN readings_type ON readings.`type` = readings_type.id "
			+ "WHERE readings_type.id = 2";

	public GasReadingContext() throws SQLException
	{
		super();
	}
	...
	private List<AbstractReading> stripResults() throws SQLException
	{
		...
		{
			AbstractReading g = new GasReading();
